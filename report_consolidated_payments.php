<?php	
	...
	if($_POST['form_action'] == 'export_report'){
		...
		$reports_cols = array('Centre Id', 'Centre', 'Sector', 'Course Name', 'Course Code', 'Batch No/Code', 'Batch Start Date', 'Batch End Date', 'Course Type Residential/Non Residential', 'Studuent Registration Number', 'Candidate Name', 'Candidate Status', 'Course Fees', 'Waiver Amount', 'Fees Payable', 'Total Fee Paid', 'GST', 'Balance Fees Due');

		$sub_reports_cols = array('SN#', 'Payment Type', 'Payment Mode', 'Cheque No', 'Bank Name', 'Bank Branch', 'Deposit Date', 'Amount', 'Status', 'Receipt');

		$sub_reports_refund_cols = array('SN#', 'Payment Type', 'Payment Mode', 'Cheque No', 'Bank Name', 'Bank Branch', 'Deposit Date', 'Amount', 'Reason', 'Review');
		...
		$excelsheet_name = 'proschool_consolidated_payment_' . time();
		...
		$sub_background_color = array(
			'fill' => array(
				'type' => PHPExcel_Style_Fill::FILL_SOLID,
				'color' => array('rgb' => 'F7D780')
			)
		);

		$text_heading_color = array(
			'fill' => array(
				'type' => PHPExcel_Style_Fill::FILL_SOLID,
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Consolidated Finance Report');
		$objPHPExcel->getActiveSheet()->getStyle('A1:' . $arr_alphabet[count($reports_cols)-1] . '1')->applyFromArray($heading_bold)->applyFromArray($text_heading_color)->getFont()->setSize(16);;
		...
		$objPHPExcel->getActiveSheet()->getStyle('A3:' . $arr_alphabet[count($reports_cols)-1] . '3')->applyFromArray($heading_bold)->applyFromArray($background_color);
			...
			while($courses = tep_db_fetch_array($courses_query)){
				...
				while($batches = tep_db_fetch_array($batches_query)){
					...
					while($students = tep_db_fetch_array($students_query)){
						...
						$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, '');
						...
						$objPHPExcel->getActiveSheet()->getStyle('A' . $rows . ':' . $arr_alphabet[$cnt_innter--] . $rows)->applyFromArray($sub_background_color);

						$student_payments_query_raw = "select sp.stud_payment_id, sp.student_id, sp.deposit_id, sp.stud_payment_type, sp.stud_payment_mode, sp.stud_payment_cheque_no, sp.stud_payment_bank_name, sp.stud_payment_bank_branch, date_format(sp.stud_payment_deposit_date, '%d-%m-%Y') as stud_payment_deposit_date, sp.stud_payment_amount, sp.stud_payment_receipt_no, sp.stud_payment_status, sp.stud_payment_added, sp.cheque_cleared, i.installment_no from " . TABLE_STUDENT_PAYMENTS . " sp left join " . TABLE_INSTALLMENTS . " i on (i.installment_id = sp.installment_id)  where sp.student_id='" . $students['student_id'] . "' ";

						if(tep_not_null($start_month) && tep_not_null($start_year) && !tep_not_null($end_month) && !tep_not_null($end_year)){
							$start_date = $start_year . "-" . $start_month . "-01";
							$student_payments_query_raw .= " and sp.stud_payment_deposit_date >= '" . $start_date . "'";
							...
							$student_payments_query_raw .= " and sp.stud_payment_deposit_date <= '" . $end_date . "'";
							...
							$student_payments_query_raw .= " and (sp.stud_payment_deposit_date >= '" . $start_date . "' and sp.stud_payment_deposit_date <= '" . $end_date . "')";
						}
						...
						if(tep_db_num_rows($student_payments_query)){
							$rows++;
							$cnt_innter = 0;
							foreach($sub_reports_cols as $column){
								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $column);
							}

							$objPHPExcel->getActiveSheet()->getStyle('A' . $rows . ':' . $arr_alphabet[count($sub_reports_cols)-1] . $rows)->applyFromArray($heading_bold);

							$rows++;

							$student_payments = array();
							$cnt_payments = 1;
							while($student_payments = tep_db_fetch_array($student_payments_query)){
								$cnt_innter = 0;

								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $cnt_payments);
								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $disp_stud_payment_type_array[$student_payments['stud_payment_type']] . ($student_payments['installment_no'] != '' ? ' - Installment : ' . $student_payments['installment_no'] : ''));
								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $student_payments['stud_payment_mode']);
								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, (in_array($student_payments['stud_payment_mode'], array('CHEQUE', 'DD', 'NEFT_RTGS')) ? $student_payments['stud_payment_cheque_no'] : '-'));
								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, (in_array($student_payments['stud_payment_mode'], array('CHEQUE', 'DD', 'NEFT_RTGS')) ? $student_payments['stud_payment_bank_name'] : '-'));
								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, (in_array($student_payments['stud_payment_mode'], array('CHEQUE', 'DD', 'NEFT_RTGS')) ? $student_payments['stud_payment_bank_branch'] : '-'));

								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, (($student_payments['stud_payment_status'] != 'NOT_DEPOSITED') ? $student_payments['stud_payment_deposit_date'] : '-'));

								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $student_payments['stud_payment_amount']);
								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $disp_stud_payment_status_array[$student_payments['stud_payment_status']] . ($student_payments['cheque_cleared'] == '1' ? ' - CLEARED' : ''));
								$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, (tep_not_null($student_payments['stud_payment_receipt_no']) ? $student_payments['stud_payment_receipt_no'] : '-'));

								$rows++;
								$cnt_payments++;
							}

							$refund_query_raw = "select refund_id, centre_id, student_id, refund_amount, refund_mode, refund_inst_no, refund_bank_name, refund_branch_name, refund_reason, refund_review, date_format(refund_added, '%d %b %Y') as refund_added from " . TABLE_REFUNDS . " where student_id='" . $students['student_id'] . "'";
							$refund_query = tep_db_query($refund_query_raw);

							$rows++;

							if(tep_db_num_rows($refund_query)){

								$cnt_innter = 0;
								foreach($sub_reports_refund_cols as $column){
									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $column);
								}

								$objPHPExcel->getActiveSheet()->getStyle('A' . $rows . ':' . $arr_alphabet[count($sub_reports_refund_cols)-1] . $rows)->applyFromArray($heading_bold);

								$rows++;

								$cnt_refund = 1;
								while($refund_array = tep_db_fetch_array($refund_query)){
									$cnt_innter = 0;

									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $cnt_refund);
									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, 'Refund');
									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $refund_array['refund_mode']);
									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, (in_array($refund_array['refund_mode'], array('CHEQUE', 'DD', 'NEFT_RTGS')) ? $refund_array['refund_inst_no'] : '-'));
									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, (in_array($refund_array['refund_mode'], array('CHEQUE', 'DD', 'NEFT_RTGS')) ? $refund_array['refund_bank_name'] : '-'));
									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, (in_array($refund_array['refund_mode'], array('CHEQUE', 'DD', 'NEFT_RTGS')) ? $refund_array['refund_branch_name'] : '-'));

									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $refund_array['refund_added']);

									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $refund_array['refund_amount']);
									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $refund_array['refund_reason']);
									$objPHPExcel->getActiveSheet()->setCellValue($arr_alphabet[$cnt_innter++] . $rows, $refund_array['refund_review']);
...
<html>
	<head>
		<title><?php echo TITLE ?> : Consolidated Finance Report</title>
									...
									<tr>
										<td valign="top">
											<table cellpadding="2" cellspacing="0" border="0" width="100%" align="" class="tab">
												<tr>
													<td class="arial18BlueN">Report - Consolidated</td>
												</tr>
												<tr>
													<td><small>Track all the Payments from Students (Not Deposited and Deposited)</small></td>
												</tr>
												<tr>
													<td><img src="<?php echo DIR_WS_IMAGES ?>pixel.gif" height="10"></td>
												</tr>
