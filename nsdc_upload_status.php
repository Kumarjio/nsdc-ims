<?php	
	...
	if(isset($action) && tep_not_null($action))
	{
		...
		$stage1_uploaded = tep_db_prepare_input($_POST['stage1_uploaded']);
		$sdms_number = tep_db_prepare_input($_POST['sdms_number']);
		$stage2_uploaded = tep_db_prepare_input($_POST['stage2_uploaded']);

		$stage1_uploaded = ($stage1_uploaded == '1' ? '1' : '0');
		$stage2_uploaded = ($stage2_uploaded == '1' ? '1' : '0');

		$arr_db_values = array(
			'stage1_uploaded' => $stage1_uploaded,
			'sdms_number' => $sdms_number,
			'stage2_uploaded' => $stage2_uploaded
		);
	...
	if($_GET['actionType'] == "edit")
	{
		...
		$info_query_raw = "select student_id, centre_id, course_id, stage1_uploaded, sdms_number, stage2_uploaded, student_status from " . TABLE_STUDENTS . " where student_id='" . $int_id . "' and is_deactivated != '1' ";
...
<html>
	<head>
		<title><?php echo TITLE ?>: NSDC Upload Status</title>
		...
		<!--
			...
			function delete_record(objForm){
				if(confirm("Are you want to delete nsdc upload status details?")){
										...
										<td valign="top">
											...
											?>
												<table cellpadding="2" cellspacing="0" border="0" width="100%" align="" class="tab">
													<tr>
														<td class="arial18BlueN">NSDC Upload Status</td>
																			...
																			<table cellpadding="0" cellspacing="0" border="0" width="100%">
																				<tr>
																					<td class="arial14LGrayBold" colspan="2">
																						<fieldset>
																							<legend>NSDC Upload Info</legend>
																							<table cellpadding="5" cellspacing="5" border="0" width="100%">
																								<tr>
																									...
																									<td class="arial12LGrayBold" colspan="5">
																										<?php foreach($arr_status as $k_status=>$v_status){?>
																											<input type="radio" name="stage1_uploaded" id="stage1_uploaded" value="<?php echo $k_status;?>" class="required" <?php echo ($info['stage1_uploaded'] == $k_status ? 'checked="checked"' : '');?>  style="width:auto;" onclick="javascript: toggle_element('stage1_uploaded', 'sdms_number');">&nbsp;<?php echo $v_status;?>&nbsp;
																										<?php } ?>&nbsp;
																										
																										<span class="arial12LGrayBold sdms_number" align="right">
																											&nbsp;SDMS number&nbsp;<font color="#ff0000">*</font>&nbsp;:
																											&nbsp;<input type="text" name="sdms_number" id="sdms_number" maxlength="50" value="<?php echo  ($dupError ? $_POST['sdms_number'] : $info['sdms_number']) ?>" class="required sdms_number">
																										</span>
																									</td>
																								</tr>
																								<tr>
																									...
																									<td class="arial12LGrayBold" colspan="5">
																										<?php foreach($arr_status as $k_status=>$v_status){?>
																											<input type="radio" name="stage2_uploaded" id="stage2_uploaded" value="<?php echo $k_status;?>" class="required" <?php echo ($info['stage2_uploaded'] == $k_status ? 'checked="checked"' : '');?>  style="width:auto;" onclick="javascript: toggle_element('stage2_uploaded', '');">&nbsp;<?php echo $v_status;?>&nbsp;
																										<?php } ?>&nbsp;
																									</td>
																								</tr>
																								...
																								<!--
																									toggle_element('stage1_uploaded', 'sdms_number');
																								//-->
																								</script>
																							</table>
																						</fieldset>
