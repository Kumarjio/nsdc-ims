<?php	
	...
	$reports_cols = array('S. No.', 'Centre Id', 'Centre Location', 'MES Sector', 'Course Name', 'Course Code', 'Batch No/Code', 'Course Duration (Month)', 'Batch Start Dt', 'Batch End Dt:', 'Handholding End Date', 'Candidate Name', 'Candidate Father\'s Name', 'Gender', 'Category', 'Minority', 'Mobile No', 'Candidate\'s District', 'Training Completed (Y/N)', 'Course Type', 'Reason for Drop Out', 'Date of Dropout', 'Bank Account Opened (Y/N)', 'Name of the Bank', 'Bank Branch', 'Bank Account No', 'Bank IFSC Code');

	if(isset($_POST['form_action']) && $_POST['form_action'] == 'export_report'){
			...
			while($courses = tep_db_fetch_array($courses_query)){
				...
				while($batches = tep_db_fetch_array($batches_query)){
					...
					while($students = tep_db_fetch_array($students_query)){
						...
						$objPHPExcel->getActiveSheet()->setCellValue(($sheet_col++) . $sheet_row, $centre['centre_unq_id']);
						...
						$objPHPExcel->getActiveSheet()->setCellValue(($sheet_col++) . $sheet_row, $students['student_full_name'] . ' ' . $students['student_middle_name'] . ' ' . $students['student_surname']);
						$objPHPExcel->getActiveSheet()->setCellValue(($sheet_col++) . $sheet_row, $students['student_father_name'] . ' ' . $students['father_middle_name'] . ' ' . $students['father_surname']);
						...
						//$objPHPExcel->getActiveSheet()->setCellValue(($sheet_col++) . $sheet_row, $students['student_account_number']);
