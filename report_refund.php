<?php	
	...
	if($_POST['form_action'] == 'export_report'){
		...
		$excelsheet_name = 'proschool_refund_' . time();
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Refund Report');
				...
				while($batches = tep_db_fetch_array($batches_query)){
					...
					while($students = tep_db_fetch_array($students_query)){
						...
						if(tep_not_null($start_month) && tep_not_null($start_year) && !tep_not_null($end_month) && !tep_not_null($end_year)){
							...
							$refund_query_raw .= " and refund_added >= '" . $start_date . "'";
							...
							$refund_query_raw .= " and refund_added <= '" . $end_date . "'";
							...
							$refund_query_raw .= " and (refund_added >= '" . $start_date . "' and refund_added <= '" . $end_date . "')";
						}
...
<html>
	<head>
		<title><?php echo TITLE ?> : Refund Report</title>
									...
									<tr>
										<td valign="top">
											<table cellpadding="2" cellspacing="0" border="0" width="100%" align="" class="tab">
												<tr>
													<td class="arial18BlueN">Report - Refund</td>
												</tr>
												<tr>
													<td><small>TO view the amount that have been refunded to the students</small></td>
												</tr>
												<tr>
													<td><img src="<?php echo DIR_WS_IMAGES ?>pixel.gif" height="10"></td>
												</tr>
