<?php	
	...
	if(isset($action) && tep_not_null($action))
	{
		...
		$non_res_alw_amt_paid = tep_db_prepare_input($_POST['non_res_alw_amt_paid']);
		$non_res_alw_date = tep_db_prepare_input($_POST['non_res_alw_date']);
		$non_res_alw_mop = tep_db_prepare_input($_POST['non_res_alw_mop']);
		$non_res_alw_installment = tep_db_prepare_input($_POST['non_res_alw_installment']);
		$non_res_alw_remarks = tep_db_prepare_input($_POST['non_res_alw_remarks']);

		$non_res_alw_date = input_valid_date($non_res_alw_date);

		$arr_db_values = array(
			'non_res_alw_amt_paid' => $non_res_alw_amt_paid,
			'non_res_alw_date' => $non_res_alw_date,
			'non_res_alw_mop' => $non_res_alw_mop,
			'non_res_alw_installment' => $non_res_alw_installment,
			'non_res_alw_remarks' => $non_res_alw_remarks
		);
		...
		switch($action){
			case 'edit':
				...
				$old_installment_query_raw = "select installment_no, receipt_filename from " . TABLE_INSTALLMENTS . " where student_id = '" . $student_id . "' and installment_type = 'NON_RES_ALLOWANCE'";
				...
				tep_db_query("delete from " . TABLE_INSTALLMENTS . " where student_id = '" . $student_id . "' and installment_type = 'NON_RES_ALLOWANCE'");

				for($cnt_inst = 1; $cnt_inst<=$non_res_alw_installment; $cnt_inst++){
					...
					$arr_db_values = array(
						...
						'installment_type' => 'NON_RES_ALLOWANCE',
				...
				$msg = 'update_res_allow';

			break;
			case 'delete':
				$arr_db_values = array(
					'non_res_alw_amt_paid' => '',
					'non_res_alw_date' => '',
					'non_res_alw_mop' => '',
					'non_res_alw_installment' => '0',
					'non_res_alw_remarks' => '',
				);
	...
	if($_GET['actionType'] == "edit")
	{
		...
		$info_query_raw = "select student_id, centre_id, course_id, non_res_alw_amt_paid, date_format(non_res_alw_date, '%d-%m-%Y') as non_res_alw_date, non_res_alw_mop, non_res_alw_installment, non_res_alw_remarks, student_status from " . TABLE_STUDENTS . " where student_id='" . $int_id . "' and is_deactivated != '1' ";
...
<html>
	<head>
		<title><?php echo TITLE ?>: Non Residential Allowance</title>
		...
		<!--
			...
			function toggle_installment(){
				var installment = $('#non_res_alw_installment').val();
			...
			function delete_record(objForm){
				if(confirm("Are you want to delete Non Residential Allowance details?")){
										...
										<td valign="top">
											<?php
												if($_GET['actionType'] == "edit")
												{
													...
													$installment_query_raw = "select installment_id, student_id, installment_type, installment_no, date_format(installment_date, '%d-%m-%Y') as installment_date, installment_mop, instrument_no, installment_amount, is_receipt_collected, receipt_filename from " . TABLE_INSTALLMENTS . " where student_id='" . $int_id . "' and installment_type = 'NON_RES_ALLOWANCE' order by installment_no";
											...
											?>
												<table cellpadding="2" cellspacing="0" border="0" width="100%" align="" class="tab">
													<tr>
														<td class="arial18BlueN">Non Residential Allowance</td>
																			...
																			<table cellpadding="0" cellspacing="0" border="0" width="100%">
																				<tr>
																					<td class="arial14LGrayBold" colspan="2">
																						<fieldset>
																							<legend>Non Residential Allowance</legend>
																							<table cellpadding="5" cellspacing="5" border="0" width="100%">
																								<tr>
																									...
																									<td class="arial12LGray">
																										<input type="text" name="non_res_alw_amt_paid" id="non_res_alw_amt_paid" maxlength="11" value="<?php echo  ($dupError ? $_POST['non_res_alw_amt_paid'] : $info['non_res_alw_amt_paid']) ?>" class="required number">
																									</td>
																								</tr>
																								<!-- <tr>
																									<td class="arial12LGrayBold" align="right">&nbsp;Date<font color="#ff0000">*</font>&nbsp;:</td>
																									<td class="arial12LGray">
																										<input type="text" name="non_res_alw_date" id="non_res_alw_date" value="<?php //echo  ($dupError ? $_POST['non_res_alw_date'] : $info['non_res_alw_date']) ?>">
																									</td>
																								</tr>
																								<tr>
																									<td class="arial12LGrayBold" align="right">&nbsp;Mode of Payment<font color="#ff0000">*</font>&nbsp;:</td>
																									<td class="arial12LGray">
																										<select name="non_res_alw_mop" id="non_res_alw_mop" class="required">
																											<option value="">Please choose</option>
																											<?php //foreach($arr_payment_type as $k_payment_type=>$v_payment_type){?>
																											<option value="<?php //echo $k_payment_type;?>" <?php //echo($info['non_res_alw_mop'] == $k_payment_type ? 'selected="selected"' : '');?>><?php e//cho $v_payment_type;?></option>
																											<?php //} ?>
																										</select>
																									</td>
																								</tr> -->
																								<tr>
																									...
																									<td class="arial12LGray">
																										<select name="non_res_alw_installment" id="non_res_alw_installment" class="required" style="width:80px;" onchange="javascript: toggle_installment();">
																											<option value="">Choose</option>
																											<?php for($cntInstallment=1;$cntInstallment<=$total_installment;$cntInstallment++){?>
																											<option value="<?php echo $cntInstallment;?>" <?php echo($info['non_res_alw_installment'] == $cntInstallment ? 'selected="selected"' : '');?>><?php echo $cntInstallment;?></option>
																											<?php } ?>
																										</select>
																									</td>
																								</tr>
																								...
																								<tr>
																									...
																									<td class="arial12LGray">
																										<textarea name="non_res_alw_remarks" id="non_res_alw_remarks" cols="40" rows="6"><?php echo  ($dupError ? $_POST['non_res_alw_remarks'] : $info['non_res_alw_remarks']) ?></textarea>
																									</td>
																								</tr>
																							</table>
																						</fieldset>
